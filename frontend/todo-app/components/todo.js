...
export default function ToDo(props) {
    ...
    const [poem, setPoem] = useState(null); // Define poem state
    const [isPoemVisible, setIsPoemVisible] = useState(false); //Track the visibility of poem box

    // The following function is added to write poem using LangChain
    async function generatePoem(id) {
        ...
        if (res.ok) {
            ...
            setPoem(data.poem);
            setIsPoemVisible(true); // Show the poem box when poem is generated
        }
    }
    ...
    return (
        <div className={styles.toDoRow} key={todo.id}>
            ...
            <button className={styles.generatePoemBtn} onClick={() => generatePoem(todo.id)}>Generate Poem</button>
